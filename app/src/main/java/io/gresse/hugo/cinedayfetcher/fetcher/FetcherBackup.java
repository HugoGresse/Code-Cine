 ...
 */
public class FetcherBackup extends AsyncTask<String, String, String> {

    private static final String TAG     = FetcherBackup.class.getSimpleName();
    ...
    FetcherBackup(@Nullable Listener listener) {
    ...
    protected String doInBackground(String... params) {
        try {
            ...
            postProgressOnMainThread(0, STEP);
...
//            Connection.Response loginForm = Jsoup.connect("https://ssl-sso.orange.fr/authM/bin/omlForm.cgi?return_url=http://cineday.orange.fr/cineday/")
            Connection.Response loginForm = Jsoup.connect("https://id.orange.fr/auth_user/bin/auth_user.cgi?return_url=http://cineday.orange.fr/cineday/")
                    .userAgent(Configuration.USER_AGENT)
                    .timeout(TIMEOUT)
                    .method(Connection.Method.GET)
                    .execute();

            postProgressOnMainThread(1, STEP);

            Connection.Response cinedayHomeResponse = Jsoup.connect("https://id.orange.fr/auth_user/bin/auth_user.cgi?return_url=http://cineday.orange.fr/cineday/")
                    .userAgent(Configuration.USER_AGENT)
            ...
            postProgressOnMainThread(2, STEP);

            Connection.Response responseCineday = Jsoup.connect("https://sso.orange.fr/pushms/advise/1.1/proposal?targets=NEC[contract:9,transverse:1]&canal=06o&data[canalPhysique]=web")
                    .userAgent(Configuration.USER_AGENT)
            ...
            postProgressOnMainThread(3, STEP);

            Connection.Response finalResponse = Jsoup.connect("https://sso.orange.fr/pushms/advise/1.1/proposal?targets=NEC[contract:9,transverse:1]&canal=06o&data[canalPhysique]=web")
                    .userAgent(Configuration.USER_AGENT)
                    .data("portfolioId", "0")
            ...
            if (TextUtils.isEmpty(finalResponse.cookie("wassup"))) {
                ...
                postFinishOnMainThread(false, "Identifiants non valides", null);
                return "Identifiants non valides";
            }

            Document cinedayReceiver = finalResponse.parse();

            postProgressOnMainThread(4, STEP);
            ...
            if (elements.size() > 0) {
                ...
                postFinishOnMainThread(true, cineday, null);
                return cineday;
            }
            ...
            postFinishOnMainThread(false, error, null);

            return error;
        } catch (IOException e) {
            Log.e(TAG, "Fail to retrieve cineday, exception ", e);

            postFinishOnMainThread(false, e.toString(), null);
    ...
    private void postProgressOnMainThread(final int step, final int number) {
        Log.d(TAG, "progress : " + step + " number: " + number);
    ...
    private void postFinishOnMainThread(final boolean success, final String result, final @Nullable Exception e) {
        mMainHandler.post(new Runnable() {
            @Override
            public void run() {
                if (mListener != null) {
